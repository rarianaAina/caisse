...
export function HistoryScreen({ session, db, sync }: HistoryScreenProps) {
  ...
  return (
    <div className="space-y-6">
      <EnTetePage
        titre="Historique"
        sous={`${String(sales.length)} ticket${sales.length > 1 ? 's' : ''} le ${new Date(`${day}T12:00:00`).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long' })}`}
        actions={
          <>
            <Champ label="Journée">
              {(id) => (
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_24rem]">
        ...
        <aside className="h-fit carte p-6">
          {selected ? (
            <>
              <h2 className="text-base font-semibold text-ardoise-900">
                {selected.sale.receiptNumber}
              </h2>
              ...
              {canRefund && selected.sale.refundOfSaleId === null && (
                <button
                  type="button"
                  disabled={
                    busy || (refunded.get(selected.sale.id) ?? 0) >= selected.sale.totalCents
                  }
